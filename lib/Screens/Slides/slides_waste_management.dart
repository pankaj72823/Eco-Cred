...
class SlidesWasteManagement extends ConsumerWidget {
  const SlidesWasteManagement({super.key});
  ...
  Widget build(BuildContext context,  WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Waste Management Questions',
          style: TextStyle(
            color: Colors.white,
      ...
      body: questionsAsyncValue.when(
        ...
        data: (questions) {
          final transportationQuestions = questions.where((question)=> question['category'] == 'waste_management').toList();
          return ListView.builder(
            itemCount: transportationQuestions.length,
            itemBuilder: (context, index) {
              var question = transportationQuestions[index];
              return Slides(question: question, isFinalQuestion: index== transportationQuestions.length -1,category: 'waste_management');
        },
      );
    },
      ),
